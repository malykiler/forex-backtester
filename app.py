...
st.caption("Symulator strategii hedgingowej z dokładnym śledzeniem TP/SL")
...
def run_backtest(df, initial_capital, base_lot, tp_pips, sl_pips, multiplier, pip_val, pip_sz):
    ...
    for idx, row in df.iterrows():
        ...
        if closed_this_tick:
            ...
            # Odfiltrowujemy zamknięte pozycje z listy aktywnych
...
if uploaded_file is not None:
    ...
    if df is not None and len(df) > 0:
        st.success(f"Plik załadowany! Para: **{selected_pair}** | Ticków: **{len(df):,}**")
        
        st.info(f"📏 Dystans TP: **{tp_pips * pip_size:.4f}** | Dystans SL: **{sl_pips * pip_size:.4f}** | Spread: **{custom_spread_pips} pipsa**")
        ...
        if st.button("🚀 Uruchom Symulację"):
            ...
            col3.metric("Liczba TP (Zyski)", tp_count)
            col4.metric("Liczba SL (Straty)", sl_count)
            
            fig = go.Figure()
            fig.add_trace(go.Scatter(y=equity_curve, mode='lines', name='Equity/Balance', line=dict(color='#00FF7F', width=2)))
            fig.update_layout(title=f"Krzywa Kapitału ({selected_pair})", xaxis_title="Zamknięcia transakcji", yaxis_title="Saldo ($)", template="plotly_dark")
